    ...
    switch($key){
		...
		case "fetch-customer-details":
			...
			<div class="disp-due" style="float:right"></div> <?php
			 ...
			 if(num_rows($result_cname)>0){
				 $row = $result_cname->fetchArray(SQLITE3_ASSOC);
				 $name = $row['name'];
			 }
			 ?>
				...
				<div id="products" class="tab-pane fade in active">
					...
					<table style="width:100%">
						...
						if(num_rows($result_prod)>0){ 
							while($row_prod=$result_prod->fetchArray(SQLITE3_ASSOC)){
				...
				<div id="payments" class="tab-pane fade">
					...
					<table style="width:100%">
						...
						if(num_rows($result_pay)>0){ 
							while($row_pay=$result_pay->fetchArray(SQLITE3_ASSOC)){
		...
		case "barcode-input":
            ...
            if(num_rows($search_barcode_result)>0){ 
                while($search_barcode_row=$search_barcode_result->fetchArray(SQLITE3_ASSOC)){ 
		...
		case "fetch-customer-data-for-Sales-return":
			...
			if(num_rows($sr_result)>0){ 
                while($sr_row=$sr_result->fetchArray(SQLITE3_ASSOC)){
					$pid = $sr_row['pid'];
					...
					if(num_rows($returned_result)>0){ 
						while($returned_row=$returned_result->fetchArray(SQLITE3_ASSOC)){
				...
				if(num_rows($due_result)>0){ 
					while($due_row=$due_result->fetchArray(SQLITE3_ASSOC)){
		...
		case "barcode-input-for-the-product-to-be-returned":
			...
			if(num_rows($validate_prod_result)>0){ 
                while($validate_prod_row=$validate_prod_result->fetchArray(SQLITE3_ASSOC)){
		...
		case "submit-sales-return-data":
			...
			try{
				...
				for($i=0;$i<count($prod_arr);$i++){
					...
					if($conn->query($sale_sql) == false || $conn->query($update_inv_sql) == false) throw new Exception($conn->lastErrorMsg());
				}

				if($conn->query($cash_trans_sql) == false || $bill_info_sql == false) throw new Exception($conn->lastErrorMsg());

				if($conn->query('COMMIT;')){ ?> <div class='alert alert-success'>Record Successfully Entered</div> <?php }
				else throw new Exception($conn->lastErrorMsg());
			}
			catch(Exception $e){
				$conn->query('ROLLBACK;');
				echo "<div class='alert alert-danger'>ERROR! ".$e->getMessage()."</div>";
		...
		case "Load-all-products":
			...
			if(num_rows($prod_list_result)>0){ 
				while($prod_list_row=$prod_list_result->fetchArray(SQLITE3_ASSOC)){ 
		...
		case "check-existing-entries":
			...
			$prod_list_result = $conn->query($prod_list_sql);
			if(num_rows($prod_list_result)>0){
				while($row=$prod_list_result->fetchArray(SQLITE3_ASSOC)){
					$pid = $row['pid'];
					$pname = $row['pname'];
					$pbrand = $row['brand_name'];
					$pcat = $row['ptype'];
		...
		case "Record-Transaction":
			...
			if($bill_id != 0 OR !empty($bill_id)){
				...
				if(num_rows($bill_result)>0){ 
					while($bill_row=$bill_result->fetchArray(SQLITE3_ASSOC)){
			...
			try{
				...
				if( $conn->query($bill_info_sql) == false || $conn->query($transaction_sql) == false ) throw new Exception($conn->lastErrorMsg());
		...
		case "suggest-bill-numbers":
			...
			if(num_rows($result_bill_sug)>0){
				while($row_bill_sug=$result_bill_sug->fetchArray(SQLITE3_ASSOC)){
			...
			if(!empty($update_string)){
				...
				try{
					...
					for($i=0;$i<count($string_to_array);$i++){
						...
						if($conn->query($sql_sales) == false || $conn->query($sql_bill_info) == false) throw new Exception($conn->lastErrorMsg());
					}
			...
			if(num_rows($result)>0){
				while($row=$result->fetchArray(SQLITE3_ASSOC)){
					if(!empty($row['customer_id'])){
						...
						$result_cname=$conn->query($sql_cname);
						if(num_rows($result_cname)>0){
							while($row_cname=$result_cname->fetchArray(SQLITE3_ASSOC)){
								$name = $row_cname['name'];
							}
						}
					}
		...
		case "submit-data-for-the-products-exchanged":
			...
			try{
				...
				$returned_prods_details = explode("/", $returned_prods[$j]);
					...
					if($conn->query($sql_sale_ret) == FALSE OR $conn->query($sql_products) == FALSE) throw new Exception($conn->lastErrorMsg());
				...
				for($i=0;$i<count($replaced_prods);$i++){
					...
					if($conn->query($sql_sale) == FALSE OR $conn->query($sql_products) == FALSE) throw new Exception($conn->lastErrorMsg());
				}
				if($conn->query($sql_bill) == false ) throw new Exception($conn->lastErrorMsg());
				if($mop == "cash" || $mop == "" || $mop == "both"){
					if($conn->query($sql_cash) == false) throw new Exception($conn->lastErrorMsg());
				}
				if($mop == "card" || $mop == "both"){
					if($conn->query($sql_bank) == false) throw new Exception($conn->lastErrorMsg());
				}
		...
		case "load-accrued-and-outstanding-incomes":
			...
			if(num_rows($aoi_result)>0){ 
				while($aoi_row=$aoi_result->fetchArray(SQLITE3_ASSOC)){
		...
		case "load-inventory-status":
			...
			if(num_rows($is_result)>0){ 
				while($is_row=$is_result->fetchArray(SQLITE3_ASSOC)){
					$is_pid = $is_row['pid'];	
		...
		case "save-sms-settings":
			...
			//echo str_replace("[name]", $name, $sms_content);
			$sql_sale_sms = "SELECT * FROM settings WHERE id='1' AND name='sale-sms'";
			$result_sale_sms = $conn->query($sql_sale_sms);
			if(num_rows($result_sale_sms)>0){
				$sql_update = "UPDATE settings SET value='$sms_content' WHERE id='1' AND name='sale-sms'";
				if($conn->query($sql_update)){ ?> <div class="alert-success alert">Sale SMS successfully updated</div> <?php }
				else{ ?> <div class="alert-danger alert">ERROR! <?php echo $conn->lastErrorMsg(); ?></div> <?php }
			}
		break;

		case "load-saved-settings":
			$sql = "SELECT * FROM settings WHERE id='1' AND name='sale-sms'";
			$result = $conn->query($sql);
			if(num_rows($result)>0){
				while($row=$result->fetchArray(SQLITE3_ASSOC)){
					echo $row['value'];
