<?php
    ...
    switch($key){
        case "insert-product-details" :
            ...
            if(!empty($product_name) AND !empty($product_size) AND !empty($product_sprice) AND !empty($product_bprice AND !empty($product_type))){
				if(strstr($product_size, '/')){
					...
					if($product_type == "f"){
						for($i=0; $i<count($size_array); $i++){
							$matching_sql = "SELECT * FROM products WHERE ptype='f' AND pname='$product_name' AND brand_name='$brand_name' AND psize='$size_array[$i]' AND sprice='$product_sprice' AND bprice='$product_bprice' LIMIT 1";
							...
							if(num_rows($result)>0){ 
								while($row=$result->fetch_assoc()){
									...
									$update_existing_sql = "UPDATE products SET inventory=inventory+$inv_array[$i] WHERE pid='$mpid' AND psize='$size_array[$i]'";
									if($conn->query($update_existing_sql)){ ?>
										<div class="bar-label"> <?php
											$generator = new \Picqer\Barcode\BarcodeGeneratorPNG();
											echo '<div class="barcode-label"><strong>Product Name: </strong>'.$mname.' <br/> 
        ...
        case "live-rec-search" :
			...
			$fill_id = 0;
			$sql = "SELECT pid, pname FROM products WHERE (pid LIKE '%$search_inp%' OR pname LIKE '%$search_inp%')";
		...
		case "search-customer-record":
			$search_inp = $_REQUEST['search'];
			$fill_id = 0;
			$sql = "SELECT * FROM customers WHERE (mobile LIKE '%$search_inp%' OR name LIKE '%$search_inp%')";
			$result = $conn->query($sql);
			if(num_rows($result)>0){ 
				while($row=$result->fetch_assoc()){
					$name = $row['name'];
		...
		case "update-product-details":
			...
			if(!empty($product_id) AND !empty($product_name) AND !empty($product_size) AND !empty($product_sprice) AND !empty($product_bprice) AND !empty($inv)){
		...
		case "reprint-barcode":
			...
			$size = $_REQUEST['psize']; 
			$product_sprice = $_REQUEST['sprice'];  ?>
			<div style="@media print{ @print{ size: 3in 1.5in; } }" class="bar-label"> <?php
				$generator = new \Picqer\Barcode\BarcodeGeneratorPNG();
				echo '<div class="barcode-label"><strong>Product Name: </strong>'.$product_name.' <br/> 
				<strong>Brand Name: </strong>'.$brand_name.'<br/> 
		...
		case "barcode-input":
            ...
            if(num_rows($search_barcode_result)>0){ 
		...
		case "insert-sale-details-into-database":
			...
			if(!empty($cname) AND !empty($total) AND !empty($customer_id)){
				...
				$sms_content_sql = "SELECT * FROM settings";
				$sms_content_result = $conn->query($sms_content_sql);
				if(num_rows($sms_content_result)>0){ 
					while($row_sms_content=$sms_content_result->fetch_assoc()){
						$sms_id = $row_sms_content['id'];
						$sms_name = $row_sms_content['name'];
						$sms_value = $row_sms_content['value'];

						if($sms_id == 1 && $sms_name == 'sale-sms'){
							$sms_content = $sms_value;
							if($mop == "both"){
								$paid = explode(",", $paid);
								$paid = $paid[0]+$paid[1];
							} 
							$due = $total-$paid;
							if(strpos($sms_content, "[name]") !== false) $sms_content = str_replace("[name]", $cname, $sms_content);
							if(strpos($sms_content, "[mobile]") !== false) $sms_content = str_replace("[mobile]", $customer_id, $sms_content);
							if(strpos($sms_content, "[total]") !== false) $sms_content = str_replace("[total]", $total, $sms_content);
							if(strpos($sms_content, "[paid]") !== false) $sms_content = str_replace("[paid]", $paid, $sms_content);
							if(strpos($sms_content, "[due]") !== false) $sms_content = str_replace("[due]", $due, $sms_content);
							if(strpos($sms_content, "[bill]") !== false) $sms_content = str_replace("[bill]", $bill_id, $sms_content);
							if(strpos($sms_content, "[date]") !== false) $sms_content = str_replace("[date]", $curr_date, $sms_content);
						}

						if($sms_id == 2 && $sms_name == 'send-sms'){
							$sms_status = $sms_value;
						}

						if($sms_id == 3 && $sms_name == 'number_of_sms'){
							$num_sms = $sms_value;
							$content_length = strlen($sms_content);
							$sms_count = ceil($content_length / 150);
						}
					}
				}
				...
				try{
					...
					if(num_rows($validate_cust_result) == 0){
						...
						if($conn->query($sql_customer) == false) throw new Exception($conn->error);
					}
					//if mop == both
					if(!empty($total_payment)){
						if($conn->query($cash_transactions_sql) == FALSE || $conn->query($bank_transactions_sql) == FALSE) throw new Exception($conn->error);
					}
					if(!empty($transactions_sql)){
						if(!empty($paid)){
							if($conn->query($transactions_sql) == false) throw new Exception($conn->error);
						}
					}
					...
					if($conn->query($sql_bill_info)){	
		   				if(is_connected() && !empty($sms_status) && !empty($num_sms)){
		   					if(!empty($sms_content) && !empty($customer_id)){
								...
								$parampro['route'] = "TA";
								$parampro['msgtype'] = "1";
								$parampro['sms'] = $sms_content; 
								$sendsmspro = http_build_query($parampro);
								$urlpro="http://newsms.designhost.in/index.php/smsapi/httpapi/?".$sendsmspro;
								$ch=curl_init();
		...
		case "fetch-customer-data-for-Sales-return":
			...
			//$sr_sql = "SELECT a.*, b.bill_date, c.*, d.name, SUM(IF(a.status='sale',a.quantity,0)) - SUM(IF(a.status='sr',a.quantity,0)) AS qty, SUM(IF(a.status='sale',a.amount,0)) - SUM(IF(a.status='sr',a.amount,0)) AS amt FROM sales a, bill_info b, products c, customers d WHERE a.bill_id='$bill_no' AND b.bill_id='$bill_no' AND c.pid=a.products AND a.customer_id=d.mobile GROUP BY a.bill_id, a.products";
				$sr_sql = "SELECT a.*, b.bill_date, c.*, d.name, SUM(CASE WHEN a.status='sale' THEN a.quantity ELSE 0 END) - SUM(CASE WHEN a.status='sr' THEN a.quantity ELSE 0 END) AS qty, SUM(CASE WHEN a.status='sale' THEN a.amount ELSE 0 END) - SUM(CASE WHEN a.status='sr' THEN a.amount ELSE 0 END) AS amt FROM sales a, bill_info b, products c, customers d WHERE a.bill_id='$bill_no' AND b.bill_id='$bill_no' AND c.pid=a.products AND a.customer_id=d.mobile GROUP BY a.bill_id, a.products";
			$sr_result = $conn->query($sr_sql);
			if(num_rows($sr_result)>0){ 
		...
		case "barcode-input-for-the-product-to-be-returned":
			...
			if(num_rows($validate_prod_result)>0){ 
		...
		case "submit-sales-return-data":
			...
			try{
				...
				for($i=0;$i<count($prod_arr);$i++){
					...
					$sale_sql = "INSERT INTO sales (sale_date, bill_id, customer_id, products, quantity, discount, amount, status)
					VALUES('$curr_date','$single_array_ele[0]','$single_array_ele[1]','$single_array_ele[2]','$single_array_ele[3]','0','$single_array_ele[4]','$single_array_ele[5]')";
					$update_inv_sql = "UPDATE products SET inventory=inventory+'$single_array_ele[3]' WHERE pid='$single_array_ele[2]'";
					if($conn->query($sale_sql) == false || $conn->query($update_inv_sql) == false) throw new Exception($conn->error);
				}

				if($conn->query($cash_trans_sql) == false || $bill_info_sql == false) throw new Exception($conn->error);

				if($conn->query('COMMIT;')){ ?> <div class='alert alert-success'>Record Successfully Entered</div> <?php }
			...
			if(strstr($pid, ' / ')){
				...
				if(count($array_pid) == count($array_inv)){
					for($i=0; $i<count($array_pid); $i++){
						...
						if($conn->query($sql)==TRUE AND $conn->query($sql_update)==TRUE){
		...
		case "apply-filters":
			...
			foreach ($_REQUEST as $key => $value) {
				...
				if (!empty($columnName) AND !empty($value)) {
		...
		case "Load-all-products":
			...
			if(num_rows($prod_list_result)>0){ 
		...
		case "check-existing-entries":
            ...
            $product_sprice = $_REQUEST['sprice'];
			$product_bprice = $_REQUEST['bprice'];
			$prod_list_sql = "SELECT * FROM products WHERE pname='$product_name' AND brand_name='$brand_name' AND psize='$product_size' AND sprice='$product_sprice' AND bprice='$product_bprice' LIMIT 1";
			$prod_list_result = $conn->query($prod_list_sql);
			if(num_rows($prod_list_result)>0){
				while($row=$prod_list_result->fetch_assoc()){
					...
					$pbgst = $row['bgst'];
					$psize = $row['psize'];
					if($product_name == $pname AND $brand_name == $pbrand AND $product_size == $psize AND $product_sprice == $psprice AND $product_bprice == $pbprice){
		...
		case "load-today's-sales":
			...
			$sql = "select a.products, a.sale_date, b.pid, b.pname, b.brand_name, b.psize, b.sprice, 
					SUM(CASE WHEN a.status='sale' THEN a.quantity ELSE 0 END) - SUM(CASE WHEN a.status='sr' THEN a.quantity ELSE 0 END) AS qty, 
					SUM(CASE WHEN a.status='sale' THEN a.amount ELSE 0 END) - SUM(CASE WHEN a.status='sr' THEN a.amount ELSE 0 END) AS amt 
					FROM sales a, products b WHERE a.products=b.pid AND DATE(a.sale_date) = '$curr_date' GROUP BY a.products ORDER BY a.sale_date DESC";
			$result = $conn->query($sql); ?>
			<tr class="tab-head">
				<td>Date</td>
		...
		case "Record-Transaction":
			...
			switch($mode_of_payment){
				case "cash":
					...
					$transaction_sql = "INSERT INTO cash_transactions(transaction_id, customer_id, bill_id, remarks, type, amount, balance, transaction_date)
				...
				case "card":
					...
					$transaction_sql = "INSERT INTO bank_transactions(btransaction_id, bcustomer_id, bbill_id, bremarks, btype, bamount, bbalance, btransaction_date)
			...
			try{
				...
				if( $conn->query($bill_info_sql) == false || $conn->query($transaction_sql) == false ) throw new Exception($conn->error);
			...
			if(num_rows($vc_cust_result)>0){ 
				...
				if(num_rows($result)>0){ 
					while($row=$result->fetch_assoc()){
						...
						$paid = $row['paid'];
						$due = $total-$paid;
						if( !empty($due) OR $due !=0 ) echo "<div class='alert alert-warning'>&#8377 ".$due." are due to this customer</div>'";
					}
				}
			}
			$conn->close();
		...
		case "suggest-bill-numbers":
			$val = $_REQUEST['value'];
			$sql_bill_sug = "SELECT a.bill_id, a.customer_id, b.name FROM bill_info a, customers b WHERE a.customer_id = b.mobile AND (a.bill_id LIKE '%$val%' OR b.mobile LIKE '%$val%' OR b.name LIKE '%$val%')";
			$result_bill_sug = $conn->query($sql_bill_sug); ?>
			<table style="width:100%"> <?php
			if(num_rows($result_bill_sug)>0){
		...
		case "view-bill-details-for-modifications":
			...
			if(num_rows($result)>0){
				while($row=$result->fetch_assoc()){
					...
					if(!empty($quantity) OR $quantity != 0){ ?>
			...
			if(!empty($update_string)){
				...
				try{
					...
					for($i=0;$i<count($string_to_array);$i++){
						...
						if($conn->query($sql_sales) == false || $conn->query($sql_bill_info) == false) throw new Exception($conn->error);
					}
					...
					$conn->query("ROLLBACK;"); ?>
					<div class="alert alert-danger">ERROR! <?php echo $e->getMessage(); ?></div><?php
				}
			}
			$conn->close();
		break;
		...
		case "submit-data-for-the-products-exchanged":
			...
			switch($mop){
				case "both":
					...
					if($total < 0){ 
						...
						VALUES('$tid','$mob','$bill_id','Products Exchanged For Bill Number $bill_id','out','$card','$bbal','$curr_date_time')";
					}
					else{ 
						$cbal += $cash;
						$bbal += $card;
						...
						$sql_bank = "INSERT INTO bank_transactions (btransaction_id, bcustomer_id, bbill_id, bremarks, btype, bamount, bbalance, btransaction_date)
						VALUES('$tid','$mob','$bill_id','Products Exchanged For Bill Number $bill_id','in','$card','$bbal','$curr_date_time')";
					}
				break;
			...
			try{
				...
				$returned_prods_details = explode("/", $returned_prods[$j]);
					$sql_sale_ret = "INSERT INTO sales(sale_date, bill_id, customer_id, products, quantity, discount, amount, status)
					VALUES('$curr_date_time','$bill_id','$returned_prods_details[1]','$returned_prods_details[2]','$returned_prods_details[3]','0','$returned_prods_details[4]','sr')";
					$sql_products = "UPDATE products SET inventory=inventory+$returned_prods_details[3] WHERE pid='$returned_prods_details[2]'" ;
					if($conn->query($sql_sale_ret) == FALSE OR $conn->query($sql_products) == FALSE) throw new Exception($conn->error);
				...
				for($i=0;$i<count($replaced_prods);$i++){
					...
					if($conn->query($sql_sale) == FALSE OR $conn->query($sql_products) == FALSE) throw new Exception($conn->error);
				}
				if($conn->query($sql_bill) == false ) throw new Exception($conn->error);
				if($mop == "cash" || $mop == "" || $mop == "both"){
					if($conn->query($sql_cash) == false) throw new Exception($conn->error);
				}
				if($mop == "card" || $mop == "both"){
					if($conn->query($sql_bank) == false) throw new Exception($conn->error);
				}
		...
		case "save-sms-settings":
			$sms_content = $_POST['content'];
			$status = $_POST['status'];
			$sql_sale_sms = "SELECT * FROM settings";
			$result_sale_sms = $conn->query($sql_sale_sms);
			if(num_rows($result_sale_sms)>0){
				while($row=$result_sale_sms->fetch_assoc()){
					$id = $row['id'];
					$setting_name = $row['name'];
					if($id == 1 && $setting_name == 'sale-sms') $saved_sms_content = $row['value'];
					if($id == 2 && $setting_name == 'send-sms') $saved_status = $row['value'];
				}
			}
			if($sms_content != $saved_sms_content){
				$sql_content = "UPDATE settings SET value='$sms_content' WHERE id = '1' AND name = 'sale-sms' ";
			}
			if($status != $saved_status){
				$sql_status = "UPDATE settings SET value='$status' WHERE id = '2' AND name = 'send-sms' ";
			}
			try{
				$conn->query("BEGIN;");
				if(!empty($sql_content)){
					if($conn->query($sql_content) == false) throw new Exception($conn->error);
				}
				if(!empty($sql_status)){
					if($conn->query($sql_status) == false) throw new Exception($conn->error);
				}
				if($conn->query("COMMIT;")){ ?> <div class="alert alert-success">Successfully Saved</div> <?php  }
				else{ throw new Exception($conn->error); }
			}
			catch(Exception $e){
				$conn->query("ROLLBACK;");
		...
		case "load-saved-settings":
			$sql = "SELECT * FROM settings";
			$sms_content = $send_sms = "";
			$result = $conn->query($sql);
			if(num_rows($result)>0){
				while($row=$result->fetch_assoc()){
					$id = $row['id'];
					...
					if($id == 3 && $setting_name == 'number_of_sms') $sms_left = $row['value'];
					$sms_length = strlen($sms_content);
					$num_sms = ceil($sms_length / 150);
				}
			} 
			echo json_encode( array(
			  ...
			  'numSms' => $num_sms,
			  'smsLeft' => number_format($sms_left)
			) );
		break;

		case "check-internet-connection":
			if(is_connected()) echo "true"; else echo "false";
		break;

		case 'check-number-of-messages':
			$sql = "SELECT * FROM  settings WHERE id='3' AND name='number_of_sms' AND value > 0";
			if(num_rows($conn->query($sql))>0) echo "true"; else echo "false";
		break;
    }
